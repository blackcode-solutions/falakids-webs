...
export default function NewPatientPage() {
  ...
  if (saved) return <div className="min-h-screen p-8"><div className="mx-auto mt-16 max-w-xl card p-10 text-center"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-emerald-50 text-3xl">✓</div><h1 className="mt-5 font-[family-name:var(--font-baloo)] text-3xl font-bold">Paciente cadastrado!</h1><p className="mt-2 text-sm text-[var(--muted)]">O perfil de João Pedro foi criado e já está disponível na lista de pacientes.</p><Link href="/patients" className="mt-7 inline-block rounded-xl bg-[var(--brand-blue)] px-6 py-3 text-sm font-semibold text-white">Voltar para pacientes</Link></div></div>;
  return <div className="min-h-screen p-5 lg:p-8">
    ...
    <div className="mb-7"><h1 className="font-[family-name:var(--font-baloo)] text-3xl font-bold">Novo paciente</h1><p className="mt-1 text-sm text-[var(--muted)]">Cadastre os dados da criança e do responsável.</p></div>
    <form onSubmit={(e) => { e.preventDefault(); setSaved(true); }} className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex justify-end gap-3 pb-5"><Link href="/patients" className="rounded-xl border border-[var(--panel-border)] bg-white px-5 py-3 text-sm font-semibold">Cancelar</Link><button className="rounded-xl bg-[var(--brand-blue)] px-6 py-3 text-sm font-semibold text-white shadow-sm hover:brightness-95">Cadastrar paciente</button></div>
    </form>
...
function Section({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) { return <section className="card p-6 lg:p-7"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#EEF2FF] text-[var(--brand-blue)]">{icon}</div><div><h2 className="font-[family-name:var(--font-baloo)] text-xl font-bold">{title}</h2><p className="text-xs text-[var(--muted)]">Preencha as informações abaixo</p></div></div>{children}</section>; }
