...
export default function ParentTasksPage() {
  ...
  return <div className="min-h-screen p-5 lg:p-8">
    <div className="mb-7 flex items-center justify-between"><div><p className="text-sm text-[var(--muted)]">João Pedro</p><h1 className="font-[family-name:var(--font-baloo)] text-3xl font-bold">Tarefas para casa</h1><p className="mt-1 text-sm text-[var(--muted)]">Ajude o João a praticar um pouquinho todos os dias. 💙</p></div><div className="rounded-2xl bg-[#FFF2DD] px-5 py-3 text-center"><p className="text-xs text-[var(--muted)]">Esta semana</p><p className="font-[family-name:var(--font-baloo)] text-xl font-bold">2/5</p></div></div>
...
function TaskCard({title,count,onDone}:{title:string;count:number;onDone:()=>void}) { return <div className="rounded-2xl border border-[var(--panel-border)] p-5"><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#EEF2FF]">🎯</span><h3 className="font-semibold">{title}</h3></div><p className="mt-3 text-sm text-[var(--muted)]">{count} atividades · Enviado pela Dra. Amanda</p></div><button onClick={onDone} className="rounded-xl bg-[var(--brand-orange)] px-4 py-2 text-sm font-semibold text-white">Começar</button></div></div> }
